...
struct matrix {
  ...
  int* arr[MAXROW];             // using an array of pointers to create a dynamic array in heap
};

void displayInputMatrix(struct matrix m[], int num_matrix)         // function to display the input matrices
{
    int x,y, a, b, i = 0;
...
struct matrix unbatchify(struct matrix k[], int kSize)      // function to multiply the matrices
{
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < num_matrix; i++) {
    ...
    for (j = 0; j < row; j++)
      m[i].arr[j] = (int*)malloc(col * sizeof(int));       // Initializing the dynamic array of pointers
    ...
    for (a = 0; a < row; a++) {
      ...
      for (b = 0; b < col; b++) {
          ...
          m[i].arr[a][b] = atoi(token);             // storing each matrix element in its position
...
fclose(file);
  ...
  // Network Programming starts from here........................................
  // int num_clients = atoi(argv[4]);
  // int batchSize = ceil(num_matrix / num_clients) + 1;;
...
// SERVER STARTS HERE.................................................................................
    ...
    // serv_addr.sin_addr.s_addr = INADDR_ANY;     //"sin_addr" specifies the internet address of the server and is set to INADDR_ANY for machines having a single IP address.
    ...
    if (bind(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)   // binding is done here (associating the server with its port)
    {
  ...
  int batchSize = ceil(num_matrix / num_clients) + 1;   // calculating the input batch size for each client to be given
  batch_size_global = batchSize;
  ...
  // {
    ...
    while (1)
    {
      for(i=0, j=0; i<num_matrix, j<num_clients ; i += batchSize, j++)
      {
        ...
        newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr, &clilen);    //the new socket descriptor returned by the accept() system call is stored in "newsockfd". 
        ...
        struct matrix k[batchSize];                       // Introducing a new variable for doing multiplication on
        // static int z = 0;
        // for(z, x=0; z < (j+1)*batchSize , x < batchSize; z += (1 + j*batchSize), x++)
        // {
        //   k[x] = m[z + j*batchSize];              // fitting the batch to be processed into the new variable
        //   if(z == num_matrix - 1)
    ...
    client through the send() and recv() system calls. */
            ...
            if(flag != 0)     // If the last batch of input matrices is not fully contained then process only some matrices
            {
              batch_size_global = flag;
              for(x = 0; x < flag; x++)
              {
                sprintf(temp, "%d %d ", k[x].matrix_row, k[x].matrix_col);
                strcat(buf, temp);      // the matrix number, row and col are being stored in the buf
                ...
                for(a=0; a< k[x].matrix_row; a++)
                {
                  for(b=0; b<k[x].matrix_col; b++)
                  {
                    ...
                    strcat(buf, temp);      // the matrix elements are being stored in the buf
                  }
                }
              }
              flag = 0;
...
// CLIENT STARTS HERE.................................................................................
    ...
    if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)    // connect with the server
    {
    ...
    char *token;
    token = strtok(buf, " ");   // dividing the buf input we got from the server into tokens

      int num_clients = atoi(argv[4]);
    // int batchSize = num_matrix/num_clients;
    int batchSize = ceil(num_matrix / num_clients) + 1;   // calculating the input batch size for each client to be given
    // struct matrix resultChild[2];       // define 2 matrixes to perform multiplication on
    // struct matrix resultChild[batch_size_global];       // define 2 matrixes to perform multiplication on
    ...
    struct matrix resultChild[batchSize];       // define 2 matrixes to perform multiplication on
    if(flag != 0)
     {
       struct matrix resultChild[flag];       // define 2 matrixes to perform multiplication on
     }
    ...
    if(flag == 0)
    {
      for(y=0;y < batchSize; y++)
      {
        ...
        // printf("\n%d %d\n", resultChild[y].matrix_row, resultChild[y].matrix_col);

        for (x = 0; x < resultChild[y].matrix_row; x++)
          resultChild[y].arr[x] = (int*)malloc(resultChild[y].matrix_col * sizeof(int));       // Initializing the dynamic array of pointers
...
// HERE MULTIPLY RESULTCHILD[0] TILL RESULTCHILD[2] AND SEND THE FINAL RESULT MATRIX TO SERVER
// THREADS START...................................................................
...
//     rc = pthread_create(&threads[t], NULL, MatrixMultiplication, (void *)t);
//     if (rc) {
...
//   pthread_exit(NULL);
  ...
  //THREADS END......................................................................
    ...
    if(flag == 0)
    {
      for(y=0; y < batchSize; y++)
              {
                sprintf(temp, " %d %d ", resultChild[y].matrix_row, resultChild[y].matrix_col);
                strcat(buf, temp);      // the matrix number, row and col are being stored in the buf
